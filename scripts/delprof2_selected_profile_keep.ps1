...
================================================================================
 ...
 AUTHOR   : Limehawk.io
 DATE      : December 2025
 USAGE    : .\delprof2_selected_profile_keep.ps1
================================================================================
 FILE     : delprof2_selected_profile_keep.ps1
...
--------------------------------------------------------------------------------
 ...
 Deletes ALL Windows user profiles EXCEPT the specified profile and protected
 ones (gaia, administrator). Uses DelProf2 utility.
...
if ([string]::IsNullOrWhiteSpace($ProfileToKeep) -or $ProfileToKeep -eq '$profile_to_keep') {
    $errorOccurred = $true
    $errorText = "- profile_to_keep is required. Set this variable in SuperOps."
}
...
Write-Host "--------------------------------------------------------------"
Write-Host "Profile to Keep : $ProfileToKeep"
...
try {
    ...
    # Build arguments - exclude the profile to keep AND protected profiles
    $delprof2Args = @("/u", "/ed:$ProfileToKeep")
...
if ($errorOccurred) {
    ...
    Write-Host "All profiles deleted except: $ProfileToKeep, $($ProtectedProfiles -join ', ')"
}
